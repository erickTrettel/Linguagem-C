...
Livro * AbrirArquivo(int *quantidadeLivros, int *capacidade);
...
void Salvar(Livro *livros, int quantidadeLivros);

int main(void){
	
	Livro *livros;
	int opc=0, quantidadeLivros=0, capacidade=0;
	int codigoLivro;
	
	livros = AbrirArquivo(&quantidadeLivros, &capacidade);
	...
	Salvar(livros, quantidadeLivros);
...
void receberInformacoes(Livro livros[CAP], int *quantidadeLivros){
	...
	if(*quantidadeLivros == CAP){
		printf("Erro ao receber informações, capacidade atingida\n");
	}else{
		fflush(stdin);
		printf(" CODIGO: ");
		scanf("%i", &codigoLivro);
...
Livro * AbrirArquivo(int *quantidadeLivros, int *capacidade){
	...
	if(arquivo == NULL){
		...
		*quantidadeLivros = 0;
		...
		livros = (Livro *) malloc(sizeof(Livro)*(*capacidade));
	}else{
		fread(quantidadeLivros, sizeof(int), 1, arquivo);

		*capacidade = (*quantidadeLivros)*2;

		livros = (Livro *)malloc(sizeof(Livro)*(*capacidade));

		fread(livros, sizeof(Livro), *quantidadeLivros, arquivo);
...
void Salvar(Livro *livros, int quantidadeLivros){
	...
	if(quantidadeLivros > 0){
		...
		fwrite(&quantidadeLivros, sizeof(int), 1, file);

		fwrite(livros, sizeof(Livro), quantidadeLivros, file);
